...
RL_DECK_DIR = os.path.join(ROOT, "agent_rl_lstm", "deck")
GA_DECK_DIR = os.path.join(ROOT, "deck_ga", "best_decks")
GENERATED_DECK_DIR = os.path.join(ROOT, "deck_generated")
PIPELINE_STATE = os.path.join(ROOT, "pipeline_lstm_state.json")
...
DEFAULT_RL_STEPS = 10_000_000     # Per iterasi (1 iterasi = 10M)
...
def phase_train_rl(iteration: int, args, total_steps: int, new_deck_dir: str, gen_deck_dir: str):
    ...
    import agent_rl_lstm.train as rl_train
    ...
    try:
        ...
        # Run
        log(f"RL Training: {total_steps} timesteps, resume from model_lstm_final.msgpack (if exists)")
...
def phase_run_ga(iteration: int, args):
    ...
    model_path = os.path.join(CHECKPOINT_DIR, "model_lstm_final.msgpack")
    ...
    db = CardDB(os.path.join(ROOT, "agent_rl_lstm", "EN_Card_Data.csv"))
...
def ensure_models_downloaded():
    ...
    model_final = os.path.join(CHECKPOINT_DIR, "model_lstm_final.msgpack")
    model_base = os.path.join(CHECKPOINT_DIR, "model_lstm_base.msgpack")
    kaggle_in = os.environ.get("KAGGLE_INPUT_DIR", "")
    alt_final = os.path.join(kaggle_in, "model_lstm_final.msgpack") if kaggle_in else ""
    
    local_exists = os.path.exists(model_final) or os.path.exists(model_base)
    alt_base = os.path.join(kaggle_in, "model_lstm_base.msgpack") if kaggle_in else ""
...
def run_pipeline(args):
    ...
    for iteration in range(state["completed_iterations"] + 1, args.iterations + 1):
        ...
        # --- RL Training Phase (resume dari model_lstm_final.msgpack) ---
        if state["phase"] == "train_rl":
            ...
            log(f"  RL Model: model_lstm_final.msgpack")
    ...
    log(f"  Model final: model_lstm_final.msgpack (siap eval)")
    log(f"{'='*70}")
