...
from tcg_core.agents import LSTMAgent, FFAgent
...
def main():
    ...
    print("=== TCG FF VS PTR GAMEPLAY ANALYSIS ===")
    print(f"Deck: Mega Charizard Y Sniper (1v1 FF vs PTR)")
    
    checkpoints_dir = os.path.join(ROOT, "checkpoints")
    model_path = os.path.join(checkpoints_dir, "model_lstm_pointer_final.msgpack")
    ff_model_path = os.path.join(checkpoints_dir, "model_final.msgpack")
    
    if not os.path.exists(model_path) or not os.path.exists(ff_model_path):
    ...
    agent_p0 = FFAgent("FF_P0", FFModel, action_mapping, ff_model_path if os.path.exists(ff_model_path) else None)
    ...
    while not done and step_count <= 300:  # Batasi 300 step agar log tidak penuh
        ...
        critic_val = getattr(agent_p0, 'last_value', 0.0) if active_player == 0 else getattr(agent_p1, 'last_value', 0.0)
