...
def parse_card(card, is_active=False, player_state: PlayerState = None) -> np.ndarray:
    ...
    if hp_val is not None:
        ...
        # Simpan fitur serangan spesifik (Attack 0 & Attack 1)
        atk0_dmg = 0.0
        atk0_cost = 0.0
        atk1_dmg = 0.0
        atk1_cost = 0.0
        ...
        if card_data and card_data.attacks:
            for idx, atk_id in enumerate(card_data.attacks):
                ...
                if atk:
                    ...
                    if attack_count == 0:
                        atk0_dmg = dmg
                        atk0_cost = len(atk.energies)
                    elif attack_count == 1:
                        atk1_dmg = dmg
                        atk1_cost = len(atk.energies)
                    attack_count += 1
        ...
        features[24] = atk0_dmg / 300.0
        features[25] = atk0_cost / 10.0
        features[26] = 1.0 if attack_count > 0 else 0.0
        features[29] = atk1_dmg / 300.0
        features[30] = atk1_cost / 10.0
...
def extract_features(state: State, select_data: SelectData, your_index: int, opp_known_hand: list = None) -> dict:
    ...
    # Slot 113-172: My Deck (60) - Hybrid Deck Tracking
