...
from agent_rl_pointer.ppo_update import ppo_update_step, get_action_and_value
...
def train():
    ...
    print(f"New Deck path (70% Meta): {NEW_DECK_PATH}")
    print(f"Gen Deck path (30% Random): {GEN_DECK_PATH}")
    print()
    ...
    params_p0 = model.init(init_rng, dummy_seq, dummy_glob, dummy_carry)
    params_p1 = model.init(init_rng, dummy_seq, dummy_glob, dummy_carry)
    ...
    # Legacy feed-forward paths (for injecting pre-trained knowledge into the LSTM)
    legacy_model_final_path = os.path.join(SAVE_DIR, "model_final.msgpack")
    legacy_model_base_path = os.path.join(SAVE_DIR, "model_base.msgpack")

    # Download via Kaggle API jika script dijalankan di Colab/Lokal (tidak ada di mount path Kaggle)
    alt_lstm_final = os.path.join(KAGGLE_INPUT_DIR, "model_lstm_pointer_final.msgpack") if KAGGLE_INPUT_DIR else ""
    if not os.path.exists(model_lstm_final_path) and not os.path.exists(model_lstm_base_path) and not os.path.exists(legacy_model_final_path):
        if not alt_lstm_final or not os.path.exists(alt_lstm_final):
    ...
    if not os.path.exists(model_lstm_final_path) and KAGGLE_INPUT_DIR:
        alt_final = os.path.join(KAGGLE_INPUT_DIR, "model_lstm_pointer_final.msgpack")
        if os.path.exists(alt_final):
            model_lstm_final_path = alt_final
            
    if not os.path.exists(model_lstm_base_path) and KAGGLE_INPUT_DIR:
        alt_base = os.path.join(KAGGLE_INPUT_DIR, "model_lstm_pointer_base.msgpack")
        if os.path.exists(alt_base):
            model_lstm_base_path = alt_base
            
    if not os.path.exists(legacy_model_final_path) and KAGGLE_INPUT_DIR:
        alt_legacy_final = os.path.join(KAGGLE_INPUT_DIR, "model_final.msgpack")
        if os.path.exists(alt_legacy_final):
            legacy_model_final_path = alt_legacy_final

    if not os.path.exists(legacy_model_base_path) and KAGGLE_INPUT_DIR:
        alt_legacy_base = os.path.join(KAGGLE_INPUT_DIR, "model_base.msgpack")
        if os.path.exists(alt_legacy_base):
            legacy_model_base_path = alt_legacy_base

    # Helper untuk merge parameter lama ke arsitektur baru (berguna saat tambah LSTM layer)
    ...
    if os.path.exists(model_lstm_final_path):
        print(f"[*] Resuming P0 from model_lstm_final: {model_lstm_final_path}")
        with open(model_lstm_final_path, 'rb') as f:
            params_p0, success = load_matching_weights(params_p0, f.read())
            if not success:
                print(f"[!] Gagal meload model_lstm_final untuk P0.")
    elif os.path.exists(model_lstm_base_path):
        print(f"[*] Resuming P0 from model_lstm_base: {model_lstm_base_path}")
        with open(model_lstm_base_path, 'rb') as f:
            params_p0, success = load_matching_weights(params_p0, f.read())
    elif os.path.exists(legacy_model_final_path):
        print(f"[*] Resuming P0 from legacy model_final: {legacy_model_final_path}")
        with open(legacy_model_final_path, 'rb') as f:
            params_p0, success = load_matching_weights(params_p0, f.read())
    elif os.path.exists(legacy_model_base_path):
        print(f"[*] Resuming P0 from legacy model_base: {legacy_model_base_path}")
        with open(legacy_model_base_path, 'rb') as f:
            params_p0, success = load_matching_weights(params_p0, f.read())
    else:
    ...
    # Load logic for P1
    if os.path.exists(model_lstm_final_path):
        print(f"[*] Resuming P1 from model_lstm_final: {model_lstm_final_path}")
        with open(model_lstm_final_path, 'rb') as f:
            params_p1, success = load_matching_weights(params_p1, f.read())
    elif os.path.exists(model_lstm_base_path):
        print(f"[*] Resuming P1 from model_lstm_base: {model_lstm_base_path}")
        with open(model_lstm_base_path, 'rb') as f:
            params_p1, success = load_matching_weights(params_p1, f.read())
    elif os.path.exists(legacy_model_final_path):
        print(f"[*] Resuming P1 from legacy model_final: {legacy_model_final_path}")
        with open(legacy_model_final_path, 'rb') as f:
            params_p1, success = load_matching_weights(params_p1, f.read())
    elif os.path.exists(legacy_model_base_path):
        print(f"[*] Resuming P1 from legacy model_base: {legacy_model_base_path}")
        with open(legacy_model_base_path, 'rb') as f:
            params_p1, success = load_matching_weights(params_p1, f.read())
    else:
    ...
    # Initialize carry for P0 and P1
    ...
    recent_wins_m_vs_m = deque(maxlen=150)
    recent_wins_m_vs_r = deque(maxlen=150)
    recent_wins_r_vs_r = deque(maxlen=150)
    p1_update_count = 0
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(N_STEPS):
            ...
            # Simpan state lama (sebelum dieksekusi) untuk dimasukkan ke buffer
            ...
            _, _, values_sharded_p0, logits_sharded_p0, carry_repl_p0 = get_action_and_value(
                params_repl_p0, model.apply, next_seq_sharded, next_glob_sharded, carry_repl_p0, step_rngs
            )
            _, _, values_sharded_p1, logits_sharded_p1, carry_repl_p1 = get_action_and_value(
                params_repl_p1, model.apply, next_seq_sharded, next_glob_sharded, carry_repl_p1, step_rngs
            )
            ...
            for i, d in enumerate(dones):
                if d:
                    ...
                    # RESET CARRY UNTUK ENVIRONMENT YANG SELESAI (DONE)
                    carry_c_np_p0[i] = 0.0
                    carry_h_np_p0[i] = 0.0
                    carry_c_np_p1[i] = 0.0
                    carry_h_np_p1[i] = 0.0

            # Kembalikan carry ke bentuk tensor yang sharded
            carry_repl_p0 = (
            ...
            # Serta pass old_carry yang diambil sebelum step dijalankan
            buffer.add(
        ...
        if update % 1 == 0:
            ...
            if rolling_win_p0 >= 60.0 and len(recent_wins_p0) == recent_wins_p0.maxlen:
                ...
                upload_to_kaggle(SAVE_DIR, message=f"Update model_lstm_pointer_final dengan winrate P0 {rolling_win_p0:.1f}% (Update #{p1_update_count})")
    ...
    env.close()
    save_checkpoint(unreplicate(params_repl_p0), "model_lstm_pointer_base.msgpack")
    save_checkpoint(unreplicate(params_repl_p0), "model_lstm_pointer_final.msgpack")
    upload_to_kaggle(SAVE_DIR, message="Final training checkpoint (LSTM Pointer)")
