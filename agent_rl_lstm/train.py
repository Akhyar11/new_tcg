...
from agent_rl_lstm.model import PokemonAgent
from agent_rl_lstm.vector_env import VectorEnv
from agent_rl_lstm.buffer import RolloutBuffer
from agent_rl_lstm.ppo_update import ppo_update_step, get_action_and_value
...
def train():
    ...
    alt_lstm_final = os.path.join(KAGGLE_INPUT_DIR, "model_lstm_final.msgpack") if KAGGLE_INPUT_DIR else ""
    ...
    if not os.path.exists(model_lstm_final_path) and KAGGLE_INPUT_DIR:
        alt_final = os.path.join(KAGGLE_INPUT_DIR, "model_lstm_final.msgpack")
    ...
    if not os.path.exists(model_lstm_base_path) and KAGGLE_INPUT_DIR:
        alt_base = os.path.join(KAGGLE_INPUT_DIR, "model_lstm_base.msgpack")
    ...
    for update in range(1, num_updates + 1):
        ...
        if update % 1 == 0:
            ...
            if rolling_win_p0 >= 60.0 and len(recent_wins_p0) == recent_wins_p0.maxlen:
                ...
                upload_to_kaggle(SAVE_DIR, message=f"Update model_lstm_final dengan winrate P0 {rolling_win_p0:.1f}% (Update #{p1_update_count})")
    ...
    env.close()
    save_checkpoint(unreplicate(params_repl_p0), "model_lstm_base.msgpack")
    save_checkpoint(unreplicate(params_repl_p0), "model_lstm_final.msgpack")
    upload_to_kaggle(SAVE_DIR, message="Final training checkpoint (LSTM)")
